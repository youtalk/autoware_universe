...
{
QPSolverResult QPSolverOSQP::solve(
  ...
  const Eigen::VectorXd & ub_a, Eigen::VectorXd & u)
{
  ...
  if (status_val != 1) {
    return QPSolverResult{false, "optimization failed : " + osqpsolver_.getStatusMessage()};
  }
  const auto has_nan =
    std::any_of(U_osqp.begin(), U_osqp.end(), [](const auto v) { return std::isnan(v); });
  if (has_nan) {
    return QPSolverResult{false, "optimization failed: result contains NaN values"};
  }
  ...
  if (status_polish == -1 || status_polish == 0) {
    ...
    return QPSolverResult{
      true, std::string(s) + " The required accuracy is met, but the solution can be inaccurate."};
  }
