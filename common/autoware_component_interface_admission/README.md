...
The deploy-time gate matches on **version + `interface_name` only**. The remap-resolved `resolved_name` match (stage 2 of the rule) is runtime-only, because remaps live in the launch / compose layer and are not visible in image metadata — that residual `TOPIC_MISMATCH` is what the runtime trigger backstops.
