...
While the pointcloud check is enabled, a grid that is stale (older than `pointcloud.obstacle_grid_timeout_sec`), in the wrong frame, missing a required layer, unconvertible, or not yet received is treated as **unavailable** rather than as evidence of "clear". This affects exactly one decision: being a no-start guard, an already-latched stop is **held** rather than released while the grid is unavailable, and a fresh valid grid restores normal hysteresis-based clearing. It does not add protection in the non-stopped state — as with a dead `~/input/pointcloud` before this migration, an unavailable grid simply means no pointcloud-based check that cycle. (The all-NaN "alive" heartbeat grid is a valid, fresh grid reporting no obstacle, so it clears normally.)
