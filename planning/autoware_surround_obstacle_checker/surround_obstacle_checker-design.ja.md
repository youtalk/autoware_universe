...
自車と最近傍の障害物との距離を計算する。
点群チェックでは、自車のポリゴンと障害物グリッド（`grid_map_msgs/GridMap`、`base_link`、`autoware_obstacle_grid_extractor` が生成）の最近傍の該当セルとの距離を求める。動的物体では、自車のポリゴンと各動的物体のポリゴンとの距離を求める。セルは何らかの点を含んだ時点（`point_count >= 1`）で該当とみなし、チェックは純粋に 2D であるためセルの高さはゲートしない。該当セルは 4 隅の点として展開されるため、報告される最近傍点は該当セルの角の 1 つであり、そのセルの位置から半セル対角線（既定の分解能 0.2 m では約 0.141 m）以内に位置する。`z = 0.0` とするのは、セルが単一の高さを持たず 2D の情報として扱うためである。
