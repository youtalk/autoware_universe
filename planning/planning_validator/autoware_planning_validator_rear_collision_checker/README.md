...
!!! warning "Producer ROI must cover the rear reach"
    ...
    This checker queries obstacles far behind ego. With the shipped `adjacent_lane.participants` values (`max_velocity` 16.6 m/s, `max_deceleration` -2.0 m/s^2, `reaction_time` 1.2 s) the participant stopping distance is `1.2 * 16.6 + 16.6^2 / (2 * 2.0)` = 88.8 m; adding `pointcloud.range.buffer` (1.0 m) and the rear overhang (1.1 m for sample_vehicle) gives a backward reach of about **91 m**. That is the worst case: the ego stopping distance is subtracted from it, so the reach shrinks as ego speeds up. The stock obstacle-grid extractor is forward-biased — its reference ROI (`length_x` 60 m, `offset_x` +20 m) covers only 10 m behind base_link — so its rear coverage can be far shorter than that reach. When the received grid's rear extent is smaller than the required backward distance, cells beyond the grid are invisible and a throttled **ERROR** is logged naming both numbers. Covering the rear reach is therefore a launch-side configuration requirement that this checker cannot enforce on its own: the obstacle-grid producer must be launched with its ROI rebiased/widened towards the rear (e.g. `x` in `[-100, +50]`) so the published grid extends at least as far behind ego as this check reaches. On the default forward-biased ROI that gap stays unmonitored, so the launch configuration must be updated alongside this change.
...
The occupancy gate is, by design, **materially more permissive** than the clustering it replaces, and that direction matters. The legacy path required a cluster of at least 5 points (`min_size=5`) **and** a cluster height of at least 0.1 m before anything counted as an obstacle. The grid path requires a single raw return in one cell (`min_point_count_cell` 1) that clears the `z_floor` height test. A lone spurious return can therefore qualify where a 5-point cluster was previously needed, so the expected practical consequence is **more** spurious rear-collision holds, not fewer. This is the intended trade — it favours not missing a real rear obstacle over suppressing noise — but no measurement of the resulting rate is offered here, and deployments sensitive to false holds should raise `min_point_count_cell`.
...
| `pointcloud.obstacle_grid_timeout_sec`            | [s]     | double | Staleness watchdog; a grid older than this reads as data-unavailable and the check abstains (see the note above)     | 0.5           |
