...
### IMU path generation: yaw rate source

The IMU-based path (still named `imu_path`/`use_imu_path`, and referred to as the "IMU path" throughout this document for backward compatibility) is generated by integrating a bicycle model driven by the ego longitudinal velocity and yaw rate. The yaw rate is taken from `twist.twist.angular.z` of `/localization/kinematic_state` (a `nav_msgs/Odometry` already expressed in `base_link`), i.e. the localization-fused twist, rather than from the angular velocity of a raw IMU sensor. This removes the per-cycle TF lookup from the IMU sensor frame into `base_link` that the previous raw-IMU source required.

It has been suggested that the steering angle could be used instead of the yaw rate. The pros and cons of both approaches are:

Yaw rate (localization-fused twist):

- (+) Usually, it has high accuracy.
- (-) It depends on the localization/odometry pipeline (see [Preventing collisions in case of wrong Odometry](#preventing-collisions-in-case-of-wrong-odometry-imu-path-only) for the functional-safety implication).
...
AEB generates a predicted footprint path based on the current velocity and the current yaw rate. The yaw rate is taken from the localization-fused twist of `/localization/kinematic_state` (see [IMU path generation: yaw rate source](#imu-path-generation-yaw-rate-source)), not from a raw IMU sensor. Note that if `use_imu_path` is `false`, it skips this step. This predicted path is generated as:
