...
- A stale grid (older than `obstacle_grid_timeout_sec`), a non-`base_link` frame, or a grid missing a required layer is treated as unavailable (throttled error, no new stop from this branch) and never as "clear"; the point tracker is left untouched, so a stale frame inserts no new stop and does not age the tracker. When `enable_stop_for_pointcloud` is enabled, an unavailable grid **holds** an already-active pointcloud stop instead of releasing it: the last collision latched from an available grid keeps being applied for the duration of the outage, and only a fresh, valid grid re-latches the evidence (which may then clear the stop). Sensor silence therefore never releases the brake — raising `obstacle_grid_timeout_sec` widens the window in which a grid still counts as fresh, it does not shorten how long a stop is held once the grid is judged unavailable.
