...
## Stage-2 note (deferred)

The runtime broadcast of the manifest over `transient_local` and the runtime admission checker are **deferred to Stage 2** per the design (`component-interface-versioning.md` section 0.5 / R-IF-13, deploy-time-first re-sequencing). The runtime home of the handshake message is still an open design item, which is why the records here are plain C++ structs rather than rosidl messages — the field sets mirror the intended message set 1:1 so the later binding is mechanical. The shared `evaluate()` in this package is what a future runtime checker will reuse.

## R-IF-14 note

The OCI-label deploy-time gate presupposes **multi-container images**; native / monolithic deployments are not covered by the image-label mechanism and fall back to the runtime trigger (R-IF-14). Driving multi-container adoption is on the deploy-time track.
